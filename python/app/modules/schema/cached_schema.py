...
class CachedShotgunSchema(object):
    ...
    def get_type_display_name(cls, sg_entity_type):
        ...
        if sg_entity_type in self._type_schema:
            ...
            self._app.log_debug("Schema information requested for unknown "
                                "type '%s'. Cache will refresh." % sg_entity_type)
            self._refresh_schema()
    ...
    def get_field_display_name(cls, sg_entity_type, field_name):
        ...
        if field_name == "type":
            # 'type' doesn't seem to be returned in the fields schema
            # don't trigger a reload = it won't help
            pass
        
        elif sg_entity_type not in self._field_schema:
            self._app.log_debug("Field information requested for unknown "
                                "type '%s'. Cache will refresh." % sg_entity_type)            
            self._refresh_schema()
        
        elif field_name not in self._field_schema[sg_entity_type]:
            self._app.log_debug("Field information requested for unknown field '%s.%s'. "
                                "Cache will refresh." % (sg_entity_type, field_name))            
            self._refresh_schema()
